...
namespace Org.BouncyCastle.Asn1.X509
{
        ...
        : Asn1Encodable, IAsn1Choice
    {
            ...
            string name)
        {
            ...
            {
                var enc = toGeneralNameEncoding(name);
                if (enc == null)
        ...
        public static GeneralName GetInstance(
            object obj)
        {
            if (obj == null || obj is GeneralName) return (GeneralName)obj;

            if (obj is Asn1TaggedObject)
            {
                var tagObj = (Asn1TaggedObject)obj;
                var tag = tagObj.TagNo;
        ...
        public override string ToString()
        {
            ...
            switch (tag)
            {
                ...
                case UniformResourceIdentifier:
                    ...
                    break;
                case DirectoryName:
                    buf.Append(X509Name.GetInstance(obj));
                    break;
                default:
                    buf.Append(obj);
                    break;
            ...
            string ip)
        {
            if (NetUtils.IPAddress.IsValidIPv6WithNetmask(ip) || NetUtils.IPAddress.IsValidIPv6(ip))
            {
                ...
                {
                    var addr = new byte[32];
                    var parsedIp = parseIPv6(ip.Substring(0, slashIndex));
                    copyInts(parsedIp, addr, 0);
                    var mask = ip.Substring(slashIndex + 1);
                    if (mask.IndexOf(':') > 0)
            ...
            if (NetUtils.IPAddress.IsValidIPv4WithNetmask(ip) || NetUtils.IPAddress.IsValidIPv4(ip))
            {
                var slashIndex = ip.IndexOf('/');
        ...
        {
            var maskVal = int.Parse(mask);

            for (var i = 0; i != maskVal; i++) addr[i / 8 + offset] |= (byte)(1 << (i % 8));
        }

        private void parseIPv4(string ip, byte[] addr, int offset)
        {
            foreach (var token in ip.Split('.', '/')) addr[offset++] = (byte)int.Parse(token);
        }
        ...
        private int[] parseMask(string mask)
        {
            ...
            for (var i = 0; i != maskVal; i++) res[i / 16] |= 1 << (i % 16);
            return res;
        }
        ...
        private int[] parseIPv6(string ip)
        {
            if (Platform.StartsWith(ip, "::"))
                ip = ip.Substring(1);
            else if (Platform.EndsWith(ip, "::")) ip = ip.Substring(0, ip.Length - 1);
            ...
            if (index != val.Length)
            {
                ...
                for (var i = doubleColon; i != val.Length - (index - doubleColon); i++) val[i] = 0;
            }
