...
namespace Org.BouncyCastle.Crypto.Engines
{
        ...
        : IStreamCipher
    {
            ...
            int outOff)
        {
            ...
            for (var i = 0; i < len; i++) output[outOff + i] = (byte)(input[inOff + i] ^ GetByte());
        }
        ...
        private void Init()
        {
            ...
            for (var i = 0; i < 16; i++) w[i >> 2] |= (uint)key[i] << (8 * (i & 0x3));
            Array.Copy(w, 0, w, 4, 4);

            for (var i = 0; i < iv.Length && i < 16; i++) w[(i >> 2) + 8] |= (uint)iv[i] << (8 * (i & 0x3));
            Array.Copy(w, 8, w, 12, 4);

            for (uint i = 16; i < 1280; i++) w[i] = F2(w[i - 2]) + w[i - 7] + F1(w[i - 15]) + w[i - 16] + i;

            Array.Copy(w, 256, p, 0, 512);
            Array.Copy(w, 768, q, 0, 512);

            for (var i = 0; i < 512; i++) p[i] = Step();
            for (var i = 0; i < 512; i++) q[i] = Step();
        ...
        {
            if (idx == 0) Pack.UInt32_To_LE(Step(), buf);
            var ret = buf[idx];
            idx = (idx + 1) & 0x3;
            return ret;
        }
