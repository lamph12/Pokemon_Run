...
namespace Org.BouncyCastle.Crypto.Engines
{
        ...
        : IWrapper
    {
            ...
            ICipherParameters parameters)
        {
            ...
            {
                var pWithR = (ParametersWithRandom)parameters;
                sr = pWithR.Random;
            ...
            int length)
        {
            ...
            var keyToBeWrapped = new byte[len];

            keyToBeWrapped[0] = (byte)length;
            Array.Copy(input, inOff, keyToBeWrapped, 1, length);

            var pad = new byte[keyToBeWrapped.Length - length - 1];
            ...
            var noOfBlocks = WKCKS.Length / engine.GetBlockSize();
            var extraBytes = WKCKS.Length % engine.GetBlockSize();

            if (extraBytes != 0) throw new InvalidOperationException("Not multiple of block length");

            engine.Init(true, paramPlusIV);

            for (var i = 0; i < noOfBlocks; i++)
            {
                var currentBytePos = i * engine.GetBlockSize();
            ...
            var TEMP3 = new byte[TEMP2.Length];

            for (var i = 0; i < TEMP2.Length; i++) TEMP3[i] = TEMP2[TEMP2.Length - (i + 1)];

            // Encrypt TEMP3 in CBC mode using the KEK and an initialization vector
            // of 0x 4a dd a2 2c 79 e8 21 05. The resulting cipher text is the desired
            // result. It is 40 octets long if a 168 bit key is being wrapped.
            var param2 = new ParametersWithIV(parameters, IV2);
            ...
            int length)
        {
            ...
            */
            ...
            for (var i = 0; i < TEMP3.Length / engine.GetBlockSize(); i++)
            {
                var currentBytePos = i * engine.GetBlockSize();
            ...
            var TEMP2 = new byte[TEMP3.Length];

            for (var i = 0; i < TEMP3.Length; i++) TEMP2[i] = TEMP3[TEMP3.Length - (i + 1)];
            ...
            var LCEKPADICV = new byte[TEMP1.Length];

            Array.Copy(TEMP1, 0, LCEKPADICV, 0, TEMP1.Length);

            for (var i = 0; i < LCEKPADICV.Length / engine.GetBlockSize(); i++)
            {
                var currentBytePos = i * engine.GetBlockSize();
            ...
            var result = new byte[LCEKPADICV.Length - 8];
