        ...
        : IBlockCipher
    {
		...
		*/
            ...
            byte[] box)
        {
            ...
            for (var j = 0; j < BC; j += 8) res |= (long)(box[(int)((r >> j) & 0xff)] & 0xff) << j;
		...
		*/
            ...
            byte[] key)
        {
            ...
            for (var i = 0; i < key.Length; i++) tk[i % 4, i / 4] = key[index++];
            ...
            {
                for (var i = 0; i < 4; i++) tk[i, 0] ^= S[tk[(i + 1) % 4, KC - 1] & 0xff];
                ...
                else
                {
                    ...
                    for (var i = 0; i < 4; i++)
                        tk[i, j] ^= tk[i, j - 1];
                    for (var i = 0; i < 4; i++) tk[i, 4] ^= S[tk[i, 3] & 0xff];
                    for (var j = 5; j < KC; j++)
                    for (var i = 0; i < 4; i++)
                        tk[i, j] ^= tk[i, j - 1];
                }
