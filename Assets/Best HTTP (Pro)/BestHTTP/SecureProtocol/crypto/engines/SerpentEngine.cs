...
namespace Org.BouncyCastle.Crypto.Engines
{
        ...
        : SerpentEngineBase
    {
        ...
        protected override int[] MakeWorkingKey(byte[] key)
        {
            ...
            for (off = 0; off + 4 < key.Length; off += 4) kPad[length++] = (int)Pack.LE_To_UInt32(key, off);

            if (off % 4 == 0)
            {
                kPad[length++] = (int)Pack.LE_To_UInt32(key, off);
                if (length < 8) kPad[length] = 1;
            }
            else
            {
            ...
            // compute w8 to w136
            //
            for (var i = 8; i < amount; i++) w[i] = RotateLeft(w[i - 8] ^ w[i - 5] ^ w[i - 3] ^ w[i - 1] ^ PHI ^ i, 11);
