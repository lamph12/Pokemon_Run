	...
	*/
        ...
        : IBlockCipher
    {
        ...
        */
        ...
        private readonly uint[] ke = new uint[6 * 2]; // for FL and FL^(-1)
        private readonly uint[] kw = new uint[4 * 2]; // for whitening
        private readonly uint[] state = new uint[4]; // for encryption and decryption

        private readonly uint[] subkey = new uint[24 * 4];
        ...
        {
            uint word = 0;
            for (var i = 0; i < 4; i++) word = (word << 8) + src[i + offset];
            return word;
        }
        ...
        private void setKey(bool forEncryption, byte[] key)
        {
            ...
            for (var i = 0; i < 4; i++) ka[i] = k[i] ^ k[i + 4];
            /* compute KA */
            camelliaF2(ka, SIGMA, 0);
            for (var i = 0; i < 4; i++) ka[i] ^= k[i];
            ...
            else
            {
                // 192bit or 256bit
                /* compute KB */
                for (var i = 0; i < 4; i++) kb[i] = ka[i] ^ k[i + 4];
