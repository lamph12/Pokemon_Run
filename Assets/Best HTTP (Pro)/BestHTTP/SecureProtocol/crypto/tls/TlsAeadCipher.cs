...
namespace Org.BouncyCastle.Crypto.Tls
{
        ...
        : TlsCipher
    {
        ...
        /// <exception cref="IOException"></exception>
        internal TlsAeadCipher(TlsContext context, IAeadBlockCipher clientWriteCipher,
            ...
            int cipherKeySize, int macSize, int nonceMode)
        {
            ...
            var dummyNonce = new byte[fixed_iv_length + record_iv_length];
        ...
        {
            var nonce = new byte[encryptImplicitNonce.Length + record_iv_length];
            ...
            switch (nonceMode)
            {
                ...
                case NONCE_DRAFT_CHACHA20_POLY1305:
                    ...
                    for (var i = 0; i < encryptImplicitNonce.Length; ++i) nonce[i] ^= encryptImplicitNonce[i];
                    break;
                default:
                    throw new TlsFatalAlert(AlertDescription.internal_error);
            }

            var plaintextOffset = offset;
            var plaintextLength = len;
            var ciphertextLength = encryptCipher.GetOutputSize(plaintextLength);

            var output = new byte[record_iv_length + ciphertextLength];
            if (record_iv_length != 0) Array.Copy(nonce, nonce.Length - record_iv_length, output, 0, record_iv_length);
            var outputPos = record_iv_length;
        ...
        public virtual byte[] DecodeCiphertext(long seqNo, byte type, byte[] ciphertext, int offset, int len)
        {
            ...
            var nonce = new byte[decryptImplicitNonce.Length + record_iv_length];
            ...
            switch (nonceMode)
            {
                ...
                case NONCE_DRAFT_CHACHA20_POLY1305:
                    TlsUtilities.WriteUint64(seqNo, nonce, nonce.Length - 8);
                    for (var i = 0; i < decryptImplicitNonce.Length; ++i) nonce[i] ^= decryptImplicitNonce[i];
                    break;
                default:
                    throw new TlsFatalAlert(AlertDescription.internal_error);
            }

            var ciphertextOffset = offset + record_iv_length;
            var ciphertextLength = len - record_iv_length;
            var plaintextLength = decryptCipher.GetOutputSize(ciphertextLength);
            ...
            try
            {
                ...
                outputPos +=
                    decryptCipher.ProcessBytes(ciphertext, ciphertextOffset, ciphertextLength, output, outputPos);
