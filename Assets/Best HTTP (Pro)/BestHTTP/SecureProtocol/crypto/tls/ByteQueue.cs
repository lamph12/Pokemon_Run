    ...
    public class ByteQueue
    {
            ...
            int skip)
        {
            if (buf.Length - offset < len)
                throw new ArgumentException("Buffer size of " + buf.Length + " is too small for a read of " + len +
                                            " bytes");
            if (available - skip < len) throw new InvalidOperationException("Not enough data to read");
            ...
            int len)
        {
            ...
            {
                var desiredSize = NextTwoPow(available + len);
