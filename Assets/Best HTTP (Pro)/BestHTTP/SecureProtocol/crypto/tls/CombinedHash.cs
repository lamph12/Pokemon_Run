...
namespace Org.BouncyCastle.Crypto.Tls
{
        ...
        : TlsHandshakeHash
    {
        ...
        {
            mMd5 = TlsUtilities.CreateHash(HashAlgorithm.md5);
            mSha1 = TlsUtilities.CreateHash(HashAlgorithm.sha1);
        }

        internal CombinedHash(CombinedHash t)
        {
            mContext = t.mContext;
            mMd5 = TlsUtilities.CloneHash(HashAlgorithm.md5, t.mMd5);
            mSha1 = TlsUtilities.CloneHash(HashAlgorithm.sha1, t.mSha1);
        }
        ...
        public virtual void TrackHashAlgorithm(byte hashAlgorithm)
        {
            throw new InvalidOperationException(
                "CombinedHash only supports calculating the legacy PRF for handshake hash");
        }
        ...
        public virtual int DoFinal(byte[] output, int outOff)
        {
            ...
            var i1 = mMd5.DoFinal(output, outOff);
            var i2 = mSha1.DoFinal(output, outOff + i1);
            return i1 + i2;
        }
        ...
        {
            var master_secret = mContext.SecurityParameters.masterSecret;

            d.BlockUpdate(master_secret, 0, master_secret.Length);
            d.BlockUpdate(ipad, 0, padLength);

            var tmp = DigestUtilities.DoFinal(d);
