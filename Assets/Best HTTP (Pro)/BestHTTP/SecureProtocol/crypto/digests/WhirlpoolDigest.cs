...
namespace Org.BouncyCastle.Crypto.Digests
{
        ...
        : IDigest, IMemoable
    {
        ...
        public int DoFinal(byte[] output, int outOff)
        {
            ...
            for (var i = 0; i < 8; i++) convertLongToByteArray(_hash[i], output, outOff + i * 8);
        ...
        public void Update(byte input)
        {
            ...
            if (_bufferPos == _buffer.Length) processFilledBuffer();
        ...
        private void processFilledBuffer()
        {
            ...
            for (var i = 0; i < _state.Length; i++) _block[i] = bytesToLongFromBuffer(_buffer, i * 8);
            processBlock();
            _bufferPos = 0;
            Array.Clear(_buffer, 0, _buffer.Length);
        }
        ...
        {
            for (var i = 0; i < 8; i++) outputArray[offSet + i] = (byte)((inputLong >> (56 - i * 8)) & 0xff);
        }
        ...
        private void processBlock()
        {
            ...
            for (var i = 0; i < 8; i++) _state[i] = _block[i] ^ (_K[i] = _hash[i]);
            ...
            for (var i = 0; i < 8; i++) _hash[i] ^= _state[i] ^ _block[i];
        }
        ...
        private byte[] copyBitLength()
        {
            ...
            for (var i = 0; i < rv.Length; i++) rv[i] = (byte)(_bitCount[i] & 0xff);
            return rv;
        }
    }
}
