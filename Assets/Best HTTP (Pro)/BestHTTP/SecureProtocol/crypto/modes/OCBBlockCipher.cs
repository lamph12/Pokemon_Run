...
namespace Org.BouncyCastle.Crypto.Modes
{
     ...
     * <a href="http://tools.ietf.org/html/rfc7253">
     *     RFC 7253 on The OCB
     ...
     * <blockquote>
     *     <p>
     *         <a href="http://www.cs.ucdavis.edu/~rogaway/ocb/license1.pdf">
     *             License for
     *             Open-Source Software Implementations of OCB
     *         </a>
     *         (Jan 9, 2013) - 'License 1'<br />
        ...
        : IAeadBlockCipher
    {
        ...
        private readonly byte[] OffsetMAIN = new byte[16];
        private readonly byte[] OffsetMAIN_0 = new byte[16];
        private readonly byte[] Stretch = new byte[24];
        private byte[] Sum;
        ...
        public virtual void Init(bool forEncryption, ICipherParameters parameters)
        {
            ...
            if (N == null) N = new byte[0];

            if (N.Length > 15) throw new ArgumentException("IV must be no more than 15 bytes");
        ...
        public virtual void ProcessAadByte(byte input)
        {
            ...
            if (++hashBlockPos == hashBlock.Length) ProcessHashBlock();
        }
        ...
        protected virtual int ProcessNonce(byte[] N)
        {
            ...
            if (KtopInput == null || !Arrays.AreEqual(nonce, KtopInput))
            {
                ...
                for (var i = 0; i < 8; ++i) Stretch[16 + i] = (byte)(Ktop[i] ^ Ktop[i + 1]);
            }
        ...
        {
            if (bs != null) Array.Clear(bs, 0, bs.Length);
        }

        protected virtual byte[] GetLSub(int n)
        {
            while (n >= L.Count) L.Add(OCB_double((byte[])L[L.Count - 1]));
            return (byte[])L[n];
        }
        ...
        {
            for (var i = 15; i >= 0; --i) block[i] ^= val[i];
        }
    }
}
