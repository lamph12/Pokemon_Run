...
namespace Org.BouncyCastle.Crypto.Modes
{
        ...
        : IAeadBlockCipher
    {
            ...
            ICipherParameters parameters)
        {
            ...
            if (cipherParameters != null) keyParam = cipherParameters;
...
#else
            var input = data.GetBuffer();
            var inLen = (int)data.Position;
#endif

            var len = ProcessPacket(input, 0, inLen, outBytes, outOff);
        ...
        public virtual int GetOutputSize(
            int len)
        {
            var totalData = (int)data.Length + len;
        ...
        public virtual int ProcessPacket(byte[] input, int inOff, int inLen, byte[] output, int outOff)
        {
            ...
            else
            {
                ...
                for (var i = macSize; i != macBlock.Length; i++) macBlock[i] = 0;
                ...
                var calculatedMacBlock = new byte[BlockSize];
        ...
        private int CalculateMac(byte[] data, int dataOff, int dataLen, byte[] macBlock)
        {
            ...
            if (HasAssociatedText()) b0[0] |= 0x40;
            ...
            if (HasAssociatedText())
            {
                ...
                var textLength = GetAssociatedTextLength();
...
#else
                    var input = associatedText.GetBuffer();
                    var len = (int)associatedText.Position;
#endif
