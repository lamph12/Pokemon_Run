...
namespace Org.BouncyCastle.Crypto.Modes
{
        ...
        : BufferedBlockCipher
    {
            ...
            int length)
        {
            ...
            if (leftOver == 0) return total - buf.Length;
        ...
        public override int ProcessBytes(
            ...
            int outOff)
        {
            if (length < 0) throw new ArgumentException("Can't have a negative input outLength!");
        ...
        public override int DoFinal(
            ...
            int outOff)
        {
            if (bufOff + outOff > output.Length) throw new DataLengthException("output buffer too small in doFinal");
            ...
            if (forEncryption)
            {
                ...
                if (bufOff < blockSize) throw new DataLengthException("need at least one block of input for CTS");

                for (var i = bufOff; i != buf.Length; i++) buf[i] = block[i - blockSize];

                for (var i = blockSize; i != bufOff; i++) buf[i] ^= block[i - blockSize];
