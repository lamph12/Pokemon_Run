...
namespace Org.BouncyCastle.Crypto.Encodings
{
        ...
        : IAsymmetricBlockCipher
    {
        ...
        private readonly byte[] fallback;
        private bool forEncryption;
        private bool forPrivateKey;
        private readonly int pLen = -1;


        private SecureRandom random;
        private readonly bool useStrictLength;

        static Pkcs1Encoding()
        {
            var strictProperty = Platform.GetEnvironmentVariable(StrictLengthEnabledProperty);

            strictLengthEnabled = new[] { strictProperty == null || strictProperty.Equals("true") };
        }
            ...
            int inLen)
        {
            ...
            if (forPrivateKey)
            {
                ...
                for (var i = 1; i != block.Length - inLen - 1; i++) block[i] = 0xFF;
            }
            else
            {
        ...
        */
            ...
            int inLen)
        {
             ...
             */
            if (pLen != -1) return DecodeBlockOrRandom(input, inOff, inLen);

            var block = engine.ProcessBlock(input, inOff, inLen);

            if (block.Length < GetOutputBlockSize()) throw new InvalidCipherTextException("block truncated");

            var type = block[0];

            if (type != 1 && type != 2) throw new InvalidCipherTextException("unknown block type");
            ...
            for (start = 1; start != block.Length; start++)
            {
                ...
                if (type == 1 && pad != 0xff) throw new InvalidCipherTextException("block padding incorrect");
            }

            start++; // data should start at the next byte

            if (start > block.Length || start < HeaderLength) throw new InvalidCipherTextException("no data in block");
