    ...
    */
        ...
        : IAsymmetricBlockCipher
    {
            ...
            byte[] encodingParams)
        {
            ...
            if (encodingParams != null) hash.BlockUpdate(encodingParams, 0, encodingParams.Length);
            ...
            int inLen)
        {
            ...
            // generate the seed.
            //
            var seed = SecureRandom.GetNextBytes(random, defHash.Length);
            ...
            for (var i = defHash.Length; i != block.Length; i++) block[i] ^= mask[i - defHash.Length];
            ...
            for (var i = 0; i != defHash.Length; i++) block[i] ^= mask[i];
            ...
            int inLen)
        {
            ...
            if (block.Length < 2 * defHash.Length + 1) throw new InvalidCipherTextException("data too short");
            ...
            {
                var diff = 0;
                for (var i = 0; i < defHash.Length; ++i) diff |= (byte)(defHash[i] ^ block[defHash.Length + i]);
