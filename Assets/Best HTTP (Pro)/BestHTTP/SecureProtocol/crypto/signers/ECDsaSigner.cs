        ...
        : IDsa
    {
        ...
        public virtual bool VerifySignature(byte[] message, BigInteger r, BigInteger s)
        {
            ...
            if (curve != null)
            {
                ...
                {
                    var D = GetDenominator(curve.CoordinateSystem, point);
                    if (D != null && !D.IsZero)
                    {
                        var X = point.XCoord;
                        while (curve.IsValidFieldElement(r))
                        {
                            var R = curve.FromBigInteger(r).Multiply(D);
                            if (R.Equals(X)) return true;
                            r = r.Add(n);
                        }
            ...
            var v = point.Normalize().AffineXCoord.ToBigInteger().Mod(n);
            return v.Equals(r);
        }
