...
namespace Org.BouncyCastle.Crypto.Signers
{
    ...
    public class Iso9796d2Signer : ISignerWithRecovery
    {
        ...
        public virtual string AlgorithmName => digest.AlgorithmName + "with" + "ISO9796-2S1";
            ...
            byte input)
        {
            ...
            if (messageLength < mBuf.Length) mBuf[messageLength] = input;
        ...
        public virtual void Reset()
        {
            ...
            if (recoveredMessage != null) ClearBlock(recoveredMessage);
        ...
        public virtual byte[] GenerateSignature()
        {
            ...
            if (delta - 1 > 0)
            {
                for (var i = delta - 1; i != 0; i--) block[i] = 0xbb;
                block[delta - 1] ^= 0x01;
                block[0] = 0x0b;
                block[0] |= header;
            }
        ...
        public virtual bool VerifySignature(byte[] signature)
        {
            ...
            // there must be at least one byte of message string
            //
            if (off - mStart <= 0) return ReturnFalse(block);
            ...
            if ((block[0] & 0x20) == 0)
            {
                ...
                if (messageLength > off - mStart) return ReturnFalse(block);
                ...
                {
                    block[off + i] ^= hash[i];
                    if (block[off + i] != 0) isOkay = false;
                }

                if (!isOkay) return ReturnFalse(block);
        ...
        private bool IsSameAs(byte[] a, byte[] b)
        {
            ...
            {
                if (mBuf.Length > b.Length) return false;

                checkLen = mBuf.Length;
            }
            else
            {
                if (messageLength != b.Length) return false;
