...
namespace Org.BouncyCastle.Crypto.Signers
{
        ...
        : ISigner
    {
            ...
            byte[] signature)
        {
            ...
            if (sig.Length == expected.Length) return Arrays.ConstantTimeAreEqual(sig, expected);
            ...
            if (sig.Length == expected.Length - 2) // NULL left out
            {
                ...
                for (var i = 0; i < hash.Length; i++) nonEqual |= sig[sigOffset + i] ^ expected[expectedOffset + i];

                for (var i = 0; i < sigOffset; i++) nonEqual |= sig[i] ^ expected[i]; // check header less NULL
