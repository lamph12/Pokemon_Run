...
namespace BestHTTP.Extensions
{
    public static class Extensions
    {
        ...
        {
            var requestPathAndQuery = uri.GetComponents(UriComponents.PathAndQuery, UriFormat.UriEscaped);
        ...
        public static string[] FindOption(this string str, string option)
        {
            ...
            var options = str.ToLower().Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
            option = option.ToLower();

            for (var i = 0; i < options.Length; ++i)
                if (options[i].Contains(option))
                    return options[i].Split(new[] { '=' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        private static readonly Regex validIpV4AddressRegex =
            new Regex("\\b(?:\\d{1,3}\\.){3}\\d{1,3}\\b", RegexOptions.IgnoreCase);
        ...
        internal static string Read(this string str, ref int pos, Func<char, bool> block, bool needResult = true)
        {
            ...
            var result = needResult ? str.Substring(startPos, pos - startPos) : null;
        ...
        internal static List<HeaderValue> ParseOptionalHeader(this string str)
        {
            ...
            {
                // Read key
                var key = str.Read(ref idx, ch => ch != '=' && ch != ',').TrimAndLower();
