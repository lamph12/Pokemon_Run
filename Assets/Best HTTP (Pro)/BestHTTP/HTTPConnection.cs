...
namespace BestHTTP
{
    ...
    {
        /// <summary>
        ///     A host sets the value of the "timeout" parameter to the time that the host will allow an idle connection to remain
        ///     open before it is closed. A connection is idle if no data is sent or received by a host.
        /// </summary>
        public TimeSpan TimeOut { get; private set; }

        /// <summary>
        ///     The "max" parameter has been used to indicate the maximum number of requests that would be made on the
        ///     connection.This parameter is deprecated.Any limit on requests can be enforced by sending "Connection: close" and
    ...
    internal enum RetryCauses
    {
        ...
        /// <summary>
        ///     If the server closed the connection while we sending a request we should reconnect and send the request again. But
        ///     we will try it once.
        /// </summary>
        Reconnect,
    ...
    internal sealed class HTTPConnection : ConnectionBase
    {
        ...
        public override bool IsRemovable
        {
            get
            {
                ...
                if (IsFree && KeepAlive != null && DateTime.UtcNow - LastProcessTime >= KeepAlive.TimeOut)
                    return true;
        ...
        {
            var alreadyReconnected = false;
            var redirected = false;

            var cause = RetryCauses.None;
                    ...
                    if (sentRequest)
                    {
                        ...
                        if (CurrentRequest.Response != null)
                        {
                            ...
                            switch (CurrentRequest.Response.StatusCode)
                            {
                                ...
                                {
                                    var authHeader =
                                        DigestStore.FindBest(
                                            CurrentRequest.Response.GetHeaderValues("www-authenticate"));
                                    if (!string.IsNullOrEmpty(authHeader))
                                    {
                                        var digest = DigestStore.GetOrCreate(CurrentRequest.CurrentUri);
                                        digest.ParseChallange(authHeader);

                                        if (CurrentRequest.Credentials != null &&
                                            digest.IsUriProtected(CurrentRequest.CurrentUri) &&
                                            (!CurrentRequest.HasHeader("Authorization") || digest.Stale))
                                ...
                                case 407:
                                {
                                    if (CurrentRequest.HasProxy)
                                    {
                                        var authHeader =
                                            DigestStore.FindBest(
                                                CurrentRequest.Response.GetHeaderValues("proxy-authenticate"));
                                        if (!string.IsNullOrEmpty(authHeader))
                                        {
                                            var digest = DigestStore.GetOrCreate(CurrentRequest.Proxy.Address);
                                            digest.ParseChallange(authHeader);

                                            if (CurrentRequest.Proxy.Credentials != null &&
                                                digest.IsUriProtected(CurrentRequest.Proxy.Address) &&
                                                (!CurrentRequest.HasHeader("Proxy-Authorization") || digest.Stale))
                                ...
                                case 308: // http://tools.ietf.org/html/rfc7238
                                {
                                    ...
                                    var location = CurrentRequest.Response.GetFirstHeaderValue("location");
                                    if (!string.IsNullOrEmpty(location))
                                    {
                                        var redirectUri = GetRedirectUri(location);

                                        if (HTTPManager.Logger.Level == Loglevels.All)
                                            HTTPManager.Logger.Verbose("HTTPConnection",
                                                string.Format("{0} - Redirected to Location: '{1}' redirectUri: '{1}'",
                                                    CurrentRequest.CurrentUri, location, redirectUri));
                                        ...
                                        if (!CurrentRequest.CallOnBeforeRedirection(redirectUri))
                                        {
                                            HTTPManager.Logger.Information("HTTPConnection",
                                                "OnBeforeRedirection returned False");
                                            goto default;
                                        }
...
#if !NETFX_CORE
                                    {
                                        throw new MissingFieldException(string.Format(
                                            "Got redirect status({0}) without 'location' header!",
                                            CurrentRequest.Response.StatusCode.ToString()));
                                    }
#else
                            ...
                            if (CurrentRequest.Response == null || !CurrentRequest.Response.IsClosedManually)
                            {
                                ...
                                var closeByServer = CurrentRequest.Response == null ||
                                                    CurrentRequest.Response.HasHeaderWithValue("connection", "close");
                                var closeByClient = !CurrentRequest.IsKeepAlive;
            ...
            finally
            {
                if (CurrentRequest != null)
                {
                    ...
                    {
                        if (CurrentRequest != null && CurrentRequest.Response != null &&
                            CurrentRequest.Response.IsUpgraded)
                            State = HTTPConnectionStates.Upgraded;
                        else
                            State = redirected ? HTTPConnectionStates.Redirected :
                                Client == null ? HTTPConnectionStates.Closed : HTTPConnectionStates.WaitForRecycle;

                        // Change the request's state only when the whole processing finished
                        if (CurrentRequest.State == HTTPRequestStates.Processing &&
                            (State == HTTPConnectionStates.Closed || State == HTTPConnectionStates.WaitForRecycle))
                        {
                            if (CurrentRequest.Response != null)
                            {
                            ...
                            {
                                CurrentRequest.Exception = new Exception(string.Format(
                                    "Remote server closed the connection before sending response header! Previous request state: {0}. Connection state: {1}",
        ...
        private void Connect()
        {
            ...
            if (!Client.Connected)
            {
                ...
                if (HTTPManager.Logger.Level == Loglevels.All)
                    HTTPManager.Logger.Verbose("HTTPConnection",
                        string.Format("'{0}' - Connecting to {1}:{2}", CurrentRequest.CurrentUri, uri.Host,
                            uri.Port.ToString()));
                ...
                if (HTTPManager.Logger.Level == Loglevels.All)
                    HTTPManager.Logger.Verbose("HTTPConnection",
                        string.Format("'{0}' - Buffer sizes - Send: {1} Receive: {2} Blocking: {3}",
                            CurrentRequest.CurrentUri, Client.SendBufferSize.ToString(),
                            Client.ReceiveBufferSize.ToString(), Client.Client.Blocking.ToString()));
#endif
            ...
            if (Stream == null)
            {
                var isSecure = HTTPProtocolFactory.IsSecureProtocol(CurrentRequest.CurrentUri);
                ...
                if (HasProxy && (!Proxy.IsTransparent || (isSecure && Proxy.NonTransparentForHTTPS)))
                {
                    ...
                    do
                    {
                        ...
                        var connectStr = string.Format("CONNECT {0}:{1} HTTP/1.1", CurrentRequest.CurrentUri.Host,
                            CurrentRequest.CurrentUri.Port);
                        ...
                        outStream.SendAsASCII(string.Format("Host: {0}:{1}", CurrentRequest.CurrentUri.Host,
                            CurrentRequest.CurrentUri.Port));
                        outStream.Write(HTTPRequest.EOL);
                        ...
                        if (HasProxy && Proxy.Credentials != null)
                            switch (Proxy.Credentials.Type)
                            {
                                    ...
                                    outStream.Write(string.Format("Proxy-Authorization: {0}",
                                        string.Concat("Basic ",
                                            Convert.ToBase64String(Encoding.UTF8.GetBytes(Proxy.Credentials.UserName +
                                                ":" + Proxy.Credentials.Password)))).GetASCIIBytes());
                                    outStream.Write(HTTPRequest.EOL);
                                    break;
                                ...
                                case AuthenticationTypes.Digest:
                                    ...
                                    {
                                        var authentication =
                                            digest.GenerateResponseHeader(CurrentRequest, Proxy.Credentials, true);
                                        if (!string.IsNullOrEmpty(authentication))
                                        {
                                            var auth = string.Format("Proxy-Authorization: {0}", authentication);
                                            if (HTTPManager.Logger.Level <= Loglevels.Information)
                                                HTTPManager.Logger.Information("HTTPConnection",
                                                    "Sending proxy authorization header: " + auth);
                        ...
                        if (HTTPManager.Logger.Level <= Loglevels.Information)
                            HTTPManager.Logger.Information("HTTPConnection",
                                "Proxy returned - status code: " + CurrentRequest.ProxyResponse.StatusCode +
                                " message: " + CurrentRequest.ProxyResponse.Message + " Body: " +
                                CurrentRequest.ProxyResponse.DataAsText);
                        ...
                        switch (CurrentRequest.ProxyResponse.StatusCode)
                        {
                            ...
                            case 407:
                            {
                                var authHeader =
                                    DigestStore.FindBest(
                                        CurrentRequest.ProxyResponse.GetHeaderValues("proxy-authenticate"));
                                if (!string.IsNullOrEmpty(authHeader))
                                {
                                    var digest = DigestStore.GetOrCreate(Proxy.Address);
                                    digest.ParseChallange(authHeader);

                                    if (Proxy.Credentials != null && digest.IsUriProtected(Proxy.Address) &&
                                        (!CurrentRequest.HasHeader("Proxy-Authorization") || digest.Stale))
                                        retry = true;
                                }
                            ...
                            default:
                                if (!CurrentRequest.ProxyResponse.IsSuccess)
                                    throw new Exception(string.Format(
                                        ...
                                        CurrentRequest.ProxyResponse.StatusCode, CurrentRequest.ProxyResponse.Message,
                                        CurrentRequest.ProxyResponse.DataAsText));
                                break;
                        }
                    } while (retry);
                ...
                if (isSecure)
                {
                    ...
                    if (CurrentRequest.UseAlternateSSL)
                    {
                        ...
                        handler.Connect(new LegacyTlsClient(CurrentRequest.CurrentUri,
                            CurrentRequest.CustomCertificateVerifyer == null
                                ? new AlwaysValidVerifyer()
                                : CurrentRequest.CustomCertificateVerifyer,
        ...
        {
            var protocol = CurrentRequest.ProtocolHandler == SupportedProtocols.Unknown
                ? HTTPProtocolFactory.GetProtocolFromUri(CurrentRequest.CurrentUri)
                : CurrentRequest.ProtocolHandler;

            if (HTTPManager.Logger.Level == Loglevels.All)
                HTTPManager.Logger.Verbose("HTTPConnection",
                    string.Format("{0} - Receive - protocol: {1}", CurrentRequest.CurrentUri, protocol.ToString()));

            CurrentRequest.Response =
                HTTPProtocolFactory.Get(protocol, CurrentRequest, Stream, CurrentRequest.UseStreaming, false);
            ...
            if (HTTPManager.Logger.Level == Loglevels.All)
                HTTPManager.Logger.Verbose("HTTPConnection",
                    string.Format("{0} - Receive - Finished Successfully!", CurrentRequest.CurrentUri));
        ...
        private bool LoadFromCache(Uri uri)
        {
            if (HTTPManager.Logger.Level == Loglevels.All)
                HTTPManager.Logger.Verbose("HTTPConnection",
                    string.Format("{0} - LoadFromCache for Uri: {1}", CurrentRequest.CurrentUri, uri));

            var cacheEntity = HTTPCacheService.GetEntity(uri);
            if (cacheEntity == null)
            {
                HTTPManager.Logger.Warning("HTTPConnection",
                    string.Format("{0} - LoadFromCache for Uri: {1} - Cached entity not found!",
                        CurrentRequest.CurrentUri, uri));
                return false;
            }
            ...
            using (var cacheStream = cacheEntity.GetBodyStream(out bodyLength))
            {
                ...
                if (!CurrentRequest.Response.HasHeader("content-length"))
                    CurrentRequest.Response.Headers.Add("content-length",
                        new List<string>(1) { bodyLength.ToString() });
        ...
        private bool TryLoadAllFromCache()
        {
            ...
            try
            {
                ...
                if (HTTPCacheService.IsCachedEntityExpiresInTheFuture(CurrentRequest))
                {
                    if (HTTPManager.Logger.Level == Loglevels.All)
                        HTTPManager.Logger.Verbose("HTTPConnection",
                            string.Format("{0} - TryLoadAllFromCache - whole response loading from cache",
                                CurrentRequest.CurrentUri));
        ...
        private void TryStoreInCache()
        {
            ...
            if (!CurrentRequest.UseStreaming &&
                ...
                HTTPCacheService.IsCacheble(CurrentRequest.CurrentUri, CurrentRequest.MethodType,
                    CurrentRequest.Response))
            {
                if (CurrentRequest.IsRedirected)
                    HTTPCacheService.Store(CurrentRequest.Uri, CurrentRequest.MethodType, CurrentRequest.Response);
                else
                    HTTPCacheService.Store(CurrentRequest.CurrentUri, CurrentRequest.MethodType,
                        CurrentRequest.Response);
            }
        }
#endif
