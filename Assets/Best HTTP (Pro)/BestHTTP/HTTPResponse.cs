...
namespace BestHTTP
{
    ...
    public class HTTPResponse : IDisposable
    {
        ...
        public virtual bool Receive(int forceReadRawContentLength = -1, bool readPayloadData = true)
        {
            ...
            catch
            {
                if (!baseRequest.DisableRetry)
                {
                    HTTPManager.Logger.Warning("HTTPResponse",
                        string.Format("{0} - Failed to read Status Line! Retry is enabled, returning with false.",
                            baseRequest.CurrentUri));
                    return false;
                }

                HTTPManager.Logger.Warning("HTTPResponse",
                    string.Format("{0} - Failed to read Status Line! Retry is disabled, re-throwing exception.",
                        baseRequest.CurrentUri));
                throw;
            }
            ...
            var versions = statusLine.Split('/', '.');
            VersionMajor = int.Parse(versions[1]);
            VersionMinor = int.Parse(versions[2]);

            if (HTTPManager.Logger.Level == Loglevels.All)
                VerboseLogging(string.Format("HTTP Version: '{0}.{1}'", VersionMajor.ToString(),
                    VersionMinor.ToString()));

            int statusCode;
            var statusCodeStr = NoTrimReadTo(Stream, (byte)' ', LF);
            ...
            {
                HTTPManager.Logger.Warning("HTTPResponse",
                    string.Format("{0} - Skipping Status Message reading!", baseRequest.CurrentUri));
        ...
        internal void ReadRaw(Stream stream, long contentLength)
        {
            ...
            var gzipped = !string.IsNullOrEmpty(encoding) && encoding == "gzip";
            ...
            {
                var buffer = new byte[Math.Max(baseRequest.StreamFragmentSize, MinBufferSize)];
                var readBytes = 0;
                ...
                while (contentLength > 0)
                {
                    ...
                    do
                    {
                        var readbuffer = (int)Math.Min(2147483646, (uint)contentLength);
                        var bytes = stream.Read(buffer, readBytes, Math.Min(readbuffer, buffer.Length - readBytes));
        ...
        {
            if (HTTPManager.Logger.Level == Loglevels.All)
                HTTPManager.Logger.Verbose("HTTPResponse", "'" + baseRequest.CurrentUri + "' - " + str);
        }
        ...
        /// <summary>
        ///     The data that downloaded from the server. All Transfer and Content encodings decoded if any(eg. chunked, gzip,
        ///     deflate).
        /// </summary>
        ...
        {
            var headerName = ReadTo(stream, (byte)':', LF).Trim();
            while (headerName != string.Empty)
            {
                var value = ReadTo(stream, LF);
        ...
        ///     (Normally, this means return a 200 response containing the full entity). In this case because of there are no
        ///     'Content-Range' header, this function will return null!
        /// </remarks>
        ...
        protected int ReadChunkLength(Stream stream)
        {
            ...
            var line = ReadTo(stream, LF);
            var splits = line.Split(';');
            var num = splits[0];

            int result;
            if (int.TryParse(num, NumberStyles.AllowHexSpecifier, null, out result))
                return result;
        ...
        protected void ReadChunked(Stream stream)
        {
            ...
            using (var output = new MemoryStream())
            {
                var chunkLength = ReadChunkLength(stream);
                ...
                while (chunkLength != 0)
                {
                    ...
                    do
                    {
                        var bytes = stream.Read(buffer, readBytes, chunkLength - readBytes);
                        if (bytes <= 0)
                            throw ExceptionHelper.ServerClosedTCPStream();
        ...
        private byte[] Decompress(byte[] data, int offset, int count, bool forceDecompress = false)
        {
            ...
            {
                decompressorGZipStream = new GZipStream(decompressorInputStream,
            ...
            var result = decompressorOutputStream.ToArray();
        ...
        protected void FeedStreamFragment(byte[] buffer, int pos, int length)
        {
            ...
            {
                var remaining = baseRequest.StreamFragmentSize - fragmentBufferDataLength;
        ...
        protected void AddStreamedFragment(byte[] buffer)
        {
            lock (SyncRoot)
            {
                ...
                if (HTTPManager.Logger.Level == Loglevels.All && buffer != null && streamedFragments != null)
                    VerboseLogging(string.Format("AddStreamedFragment buffer length: {0:N0} streamedFragments: {1:N0}",
                        buffer.Length, streamedFragments.Count));
        ...
        private bool HasFragmentsInQueue()
        {
            lock (SyncRoot)
            {
                var result = streamedFragments != null && streamedFragments.Count >= baseRequest.MaxFragmentQueueLength;
                if (result && HTTPManager.Logger.Level == Loglevels.All)
                    VerboseLogging(string.Format("HasFragmentsInQueue - {0} / {1}", streamedFragments.Count,
        ...
        public List<byte[]> GetStreamedFragments()
        {
            lock (SyncRoot)
            {
                ...
                if (HTTPManager.Logger.Level == Loglevels.All)
                    VerboseLogging(string.Format("GetStreamedFragments - returning with {0:N0} fragments",
