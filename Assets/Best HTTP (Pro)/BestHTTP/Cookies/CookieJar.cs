    ...
    public static class CookieJar
    {
            ...
            lock (Locker)
            {
                try
                {
                    ...
                    foreach (var cookieHeader in setCookieHeaders)
                        try
                        {
                            var cookie = Cookie.Parse(cookieHeader, response.baseRequest.CurrentUri);
        ...
        public static List<Cookie> Get(Uri uri)
        {
            lock (Locker)
            {
                ...
                {
                    var cookie = Cookies[i];
                    if (cookie.WillExpireInTheFuture() && uri.Host.IndexOf(cookie.Domain) != -1 &&
            ...
            lock (Locker)
            {
                ...
                for (var i = 0; i < Cookies.Count;)
                {
                    ...
                    if (cookie.Name.Equals(name, StringComparison.OrdinalIgnoreCase) &&
                        uri.Host.IndexOf(cookie.Domain) != -1)
                        Cookies.RemoveAt(i);
                    else
                        i++;
                }
