...
namespace BestHTTP
{
    ...
    public static class HTTPManager
    {
        ...
        /// <summary>
        ///     The maximum active TCP connections that the client will maintain to a server. Default value is 4. Minimum value is
        ///     1.
        /// </summary>
        public static byte MaxConnectionPerServer
        {
            get => maxConnectionPerServer;
            set
            {
                if (value <= 0)
        ...
        /// <summary>
        ///     Default value of a HTTP request's IsKeepAlive value. Default value is true. If you make rare request to the server
        ...
        /// <summary>
        ///     How many time must be passed to destroy that connection after a connection finished its last request. Its default
        ///     value is 20 seconds.
        /// </summary>
        ...
        /// <summary>
        ///     If this property is set to true, then new cookies treated as session cookies and these cookies are not saved to
        ///     disk. Its default value is false;
        /// </summary>
        public static bool EnablePrivateBrowsing { get; set; }

        /// <summary>
        ///     Global, default value of the HTTPRequest's ConnectTimeout property. If set to TimeSpan.Zero or lower, no connect
        ...
        /// <summary>
        ///     The global, default proxy for all HTTPRequests. The HTTPRequest's Proxy still can be changed per-request. Default
        ///     value is null.
        /// </summary>
        ...
        /// <summary>
        ///     A basic BestHTTP.Logger.ILogger implementation to be able to log intelligently additional informations about the
        ///     plugin's internal mechanism.
        /// </summary>
...
#if !BESTHTTP_DISABLE_ALTERNATE_SSL && (!UNITY_WEBGL || UNITY_EDITOR)
        /// <summary>
        ///     The default ICertificateVerifyer implementation that the plugin will use when the request's UseAlternateSSL
        ///     property is set to true.
        /// </summary>
        public static ICertificateVerifyer DefaultCertificateVerifyer { get; set; }

        /// <summary>
        ///     The default IClientCredentialsProvider implementation that the plugin will use when the request's UseAlternateSSL
        ///     property is set to true.
        /// </summary>
        ...
        /// <summary>
        ///     On most systems the maximum length of a path is around 255 character. If a cache entity's path is longer than this
        ///     value it doesn't get cached. There no platform independent API to query the exact value on the current system, but
        ///     it's
        ...
        /// </summary>
        private static readonly Dictionary<string, List<ConnectionBase>> Connections =
            new Dictionary<string, List<ConnectionBase>>();

        /// <summary>
        ///     Active connections. These connections all has a request to process.
        /// </summary>
        private static readonly List<ConnectionBase> ActiveConnections = new List<ConnectionBase>();

        /// <summary>
        ///     Free connections. They can be removed completely after a specified time.
        /// </summary>
        private static readonly List<ConnectionBase> FreeConnections = new List<ConnectionBase>();

        /// <summary>
        ///     Connections that recycled in the Update loop. If they are not used in the same loop to process a request, they will
        ///     be transferred to the FreeConnections list.
        /// </summary>
        private static readonly List<ConnectionBase> RecycledConnections = new List<ConnectionBase>();

        /// <summary>
        ///     List of request that have to wait until there is a free connection to the server.
        /// </summary>
        private static readonly List<HTTPRequest> RequestQueue = new List<HTTPRequest>();
        ...
        {
            var conn = FindOrCreateFreeConnection(request);
...
#if !BESTHTTP_DISABLE_PROXY
                (request.Proxy != null
                    ? new UriBuilder(request.Proxy.Address.Scheme, request.Proxy.Address.Host,
                        request.Proxy.Address.Port).Uri.ToString()
                    : string.Empty) +
#endif
                new UriBuilder(request.CurrentUri.Scheme, request.CurrentUri.Host, request.CurrentUri.Port).Uri;
        }
        ...
        private static ConnectionBase FindOrCreateFreeConnection(HTTPRequest request)
        {
            ...
            var serverUrl = GetKeyForRequest(request);
            ...
            if (Connections.TryGetValue(serverUrl, out connections))
            {
                ...
                if (activeConnections <= MaxConnectionPerServer)
                    ...
                    for (var i = 0; i < connections.Count && conn == null; ++i)
                    {
                        ...
                        if (tmpConn != null &&
                            ...
                            (
                                ...
                                tmpConn.LastProcessedUri.Host.Equals(request.CurrentUri.Host,
        ...
        /// <summary>
        ///     Update function that should be called regularly from a Unity event(Update, LateUpdate). Callbacks are dispatched
        ///     from this function.
        /// </summary>
                ...
                try
                {
                        ...
                        for (var i = 0; i < ActiveConnections.Count; ++i)
                        {
                            ...
                            switch (conn.State)
                            {
                                case HTTPConnectionStates.Processing:
                                    ...
                                    if (conn.CurrentRequest.UseStreaming && conn.CurrentRequest.Response != null &&
                                        conn.CurrentRequest.Response.HasStreamedFragments())
                                        conn.HandleCallback();

                                    try
                                    {
                                        if (((!conn.CurrentRequest.UseStreaming &&
                                              conn.CurrentRequest.UploadStream == null) ||
                                             conn.CurrentRequest.EnableTimoutForStreaming) &&
                                ...
                                case HTTPConnectionStates.TimedOut:
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            Logger.Information("HTTPManager",
                                                "Hard aborting connection because of a long waiting TimedOut state");
                    ...
                    if (Monitor.TryEnter(RecycledConnections))
                        try
                        {
                            ...
                            {
                                for (var i = 0; i < RecycledConnections.Count; ++i)
                                {
                    ...
                    if (FreeConnections.Count > 0)
                        for (var i = 0; i < FreeConnections.Count; i++)
                        {
