...
namespace BestHTTP.PlatformSupport.IO
{
 ...
 WindowsRuntimeSystemExtensions.AsTask<StorageFile>(StorageFile.GetFileFromApplicationUriAsync(uri));
            task.Wait();
        ...
        public static Stream OpenFileForWriting(string uri)
        {
            ...
            Task<StorageFile> task1 =
 WindowsRuntimeSystemExtensions.AsTask<StorageFile>(FileHelper.GetFolderForPathOrURI(uri).CreateFileAsync(fileName, CreationCollisionOption.ReplaceExisting));
            task1.Wait();
            if (task1.Status != TaskStatus.RanToCompletion)
                throw new Exception("Failed to open the file");
            Task<IRandomAccessStream> task2 =
 WindowsRuntimeSystemExtensions.AsTask<IRandomAccessStream>(task1.Result.OpenAsync(FileAccessMode.ReadWrite));
            task2.Wait();
 ...
 WindowsRuntimeSystemExtensions.AsTask<IReadOnlyList<StorageFolder>>(storageFolder1.CreateFolderQuery().GetFoldersAsync());
        ...
        internal static StorageFolder GetFolderForPathOrURI(string path)
        {
            ...
            IAsyncOperation<StorageFolder> folderFromPathAsync =
 StorageFolder.GetFolderFromPathAsync(Path.GetDirectoryName(path));
 ...
 !System.Uri.IsWellFormedUriString(path, UriKind.RelativeOrAbsolute) ? StorageFile.GetFileFromPathAsync(path) : StorageFile.GetFileFromApplicationUriAsync(new System.Uri(path));
        ...
        {
            Task<IRandomAccessStream> task =
 WindowsRuntimeSystemExtensions.AsTask<IRandomAccessStream>(file.OpenAsync(FileAccessMode.Read));
            task.Wait();
            if (task.Status != TaskStatus.RanToCompletion)
        ...
        {
            Task<IBuffer> task =
 WindowsRuntimeSystemExtensions.AsTask<IBuffer>(FileIO.ReadBufferAsync((IStorageFile)file));
            task.Wait();
            if (task.Status != TaskStatus.RanToCompletion)
