...
namespace BestHTTP.PlatformSupport.IO
{
    public static class File
    {
        ...
        public static StreamWriter AppendText(string path, Encoding encoding)
        {
            try
            {
                IAsyncOperation<StorageFile> fileAsync =
 FileHelper.GetFolderForPathOrURI(path).CreateFileAsync(Path.GetFileName(path), CreationCollisionOption.OpenIfExists);
                WindowsRuntimeSystemExtensions.AsTask<StorageFile>(fileAsync).Wait();
                IAsyncOperation<IRandomAccessStream> source =
 fileAsync.GetResults().OpenAsync(FileAccessMode.ReadWrite);
        ...
        public static string ReadAllText(string path)
        {
            ...
            {
                IAsyncOperation<IRandomAccessStream> source =
 FileHelper.GetFileForPathOrURI(path).OpenAsync(FileAccessMode.Read);
                WindowsRuntimeSystemExtensions.AsTask<IRandomAccessStream>(source).Wait();
 ...
 new StreamReader(WindowsRuntimeStreamExtensions.AsStreamForRead((IInputStream)randomAccessStream));
                ...
                if (!string.IsNullOrWhiteSpace(destinationBackupFileName))
                {
                    try
                    {
                        ...
                        if (fileName1.ToLower() == destinationFileName.ToLower())
                        {
                            ...
                            fileForPathOrUri2 =
 FileHelper.GetFileForPathOrURI(Path.Combine(Path.GetDirectoryName(fileForPathOrUri1.Path), fileName1));
                        }
                        else
                        {
                        ...
                        {
                            StorageFolder folderForPathOrUri =
 FileHelper.GetFolderForPathOrURI(destinationBackupFileName);
        ...
        {
            IAsyncOperation<StorageFile> fileAsync =
 FileHelper.GetFolderForPathOrURI(path).CreateFileAsync(Path.GetFileName(path), CreationCollisionOption.ReplaceExisting);
            WindowsRuntimeSystemExtensions.AsTask<StorageFile>(fileAsync).Wait();
