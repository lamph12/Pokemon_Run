...
public static class Helper
{
    ...
    public static T[] DisruptiveArrayObject<T>(T[] array) where T : struct
    {
        for (var i = 0; i < array.Length; i++)
        {
            ...
            var r1 = Random.Range(0, array.Length);
            var r2 = Random.Range(0, array.Length);
    ...
    public static List<object> DisruptiveListObject(List<object> array)
    {
        ...
        {
            //int r1 = System.Random.Next
            var r1 = Random.Range(0, array.Count);
            var r2 = Random.Range(0, array.Count);
            (array[r1], array[r2]) = (array[r2], array[r1]); //Swap
        }
    ...
    /// <summary>
    ///     Di chuyển theo đường cong của Animation Curve
    /// </summary>
    /// <param name="selfTransform">Transform của Object muốn di chuyển</param>
    /// <param name="desPos">Tọa độ muốn di chuyển tới</param>
    /// <param name="curve">Đường cong muốn di chuyển</param>
    /// <param name="maxHeight">Chiều cao tối đa của đường cong</param>
    /// <param name="duration">Thời gian di chuyển</param>
    /// <param name="onDone">Callback khi hoàn thành</param>
    /// <returns></returns>
    public static IEnumerator CurveMove(Transform selfTransform, Vector2 desPos, AnimationCurve curve, float maxHeight,
        float duration, Action onDone)
    {
        var time = 0f;
        var end = desPos;
        Vector2 startPoint = selfTransform.position;
        while (time <= duration)
        {
            time += Time.deltaTime;
            var linearT = time / duration;
            selfTransform.position = Vector2.Lerp(startPoint, end, linearT) +
                                     new Vector2(0f, Mathf.Lerp(0f, maxHeight, curve.Evaluate(linearT)));
            yield return null;
        }

        selfTransform.position = desPos;
        onDone?.Invoke();
    }
    ...
    /// <returns></returns>
    public static IEnumerator StartThread(UnityAction action, Func<bool> conditionStop,
    ...
    /// <returns></returns>
    public static IEnumerator StartAction(Action action, float time)
    {
    ...
    /// <param name="time"></param>
    /// <returns></returns>
    public static IEnumerator StartAction(UnityAction action, Func<bool> condition)
    {
    ...
    public static Vector3 GetDirectionFromAngle(Vector3 vectorP, float angle, Vector3 positionStart)
    {
        ...
        if (angle == 180)
            return -vectorP;
        if (angle == 360) return new Vector3(-vectorP.y, vectorP.x).normalized;
        ...
        var radiaAngle = angle * Mathf.PI / 180;
        var tanAngle = Mathf.Tan(radiaAngle);
        var u = new Vector2(vectorP.y, -vectorP.x);

        var B = GetPointDistanceFromObject(1, vectorP, positionStart);
        //Debug.Log("Distance "+ Vector3.Distance(positionStart, B));

        var result = GetPointDistanceFromObject(tanAngle, u, B);
    ...
    public static void LookAtToDirection(Vector3 diretion, GameObject objLookAt, float speedLockAt = 500)
    {
        ...
        var angle = Mathf.Atan2(yTarget, xTarget) * Mathf.Rad2Deg + 90;
        ...
        float speedLockAt = 500)
    {
        var xTarget = diretion.x;
        var yTarget = diretion.y;
        var angle = Mathf.Atan2(yTarget, xTarget) * Mathf.Rad2Deg + 90 + AngleDetal;
        var q = Quaternion.AngleAxis(angle, Vector3.forward);
        objLookAt.transform.rotation = Quaternion.Slerp(objLookAt.transform.rotation, q, Time.deltaTime * speedLockAt);
    }
    ...
    {
        var m_diff = new List<int>(diff);

        //Trộn List
        m_diff = DisruptiveListObject(m_diff);

        for (var i = 0; i < m_diff.Count; i++)
    ...
    public static List<int> SetPos(int numCount, int MaxERow)
    {
        ...
        if (numCount == 1) return new List<int> { 1 };

        if (numCount == 2) return new List<int> { 2 };

        if (numCount == 3) return new List<int> { 1, 2 };

        if (numCount == 4) return new List<int> { 2, 2 };

        if (numCount == 5) return new List<int> { 1, 2, 2 };

        if (numCount == 6) return new List<int> { 1, 2, 2, 1 };

        if (numCount == 7) return new List<int> { 1, 2, 2, 2 };

        if (numCount == 8) return new List<int> { 1, 2, 3, 2 };

        if (numCount == 9) return new List<int> { 1, 2, 3, 3 };
        ...
        var numCount_1 = numCount / 2;
        var lst_1 = PosHandle(numCount_1, MaxERow);
        result.AddRange(lst_1);

        var numCount_2 = numCount - numCount_1;
    ...
    private static List<int> PosHandle(int numCount, int MaxERow)
    {
        ...
        while (numCount > 0)
        {
            if (numCount >= hs && hs >= MaxERow) result.Add(hs);
    ...
    {
        var factor = gcd(Screen.width, Screen.height);
        var wFactor = Screen.width / factor;
        var hFactor = Screen.height / factor;
