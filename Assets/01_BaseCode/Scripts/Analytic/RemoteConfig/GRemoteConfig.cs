...
public class GRemoteConfig
{
    ...
    private static readonly Dictionary<string, string> playfabConfig = new Dictionary<string, string>();
    ...
    {
        if (!firebaseRemoteKeys.Contains(key)) return defaultValue;
        var val = FirebaseRemoteConfig.DefaultInstance.GetValue(key).StringValue;
        try
        {
    ...
    public static float PlayfabFloatConfig(string key, float defaultVal)
    {
        try
        {
            return playfabConfig.ContainsKey(key)
                ? float.Parse(playfabConfig[key], CultureInfo.InvariantCulture)
                : defaultVal;
        }
        catch (Exception)
        {
    ...
    private static void FetchComplete(Task fetchTask)
    {
        ...
        else if (fetchTask.IsCompleted) DebugLog("Fetch completed successfully!");
    ...
    public static async void FetchAndActivateAsync(Task fetchTask, ConfigInfo info)
    {
        ...
        if (!fetchTask.IsFaulted && fetchTask.IsCompleted) ReloadFirebaseKeys();
    ...
    public static void ReloadFirebaseKeys()
    {
        ...
        foreach (var key in FirebaseRemoteConfig.DefaultInstance.Keys) firebaseRemoteKeys.Add(key);
    }
