...
public class MusicManager : SerializedMonoBehaviour
{
    ...
    {
        get => PlayerPrefs.GetFloat(MASTER_KEY, 0f);
        set => SetMasterVolume(value);
    }

    public float MusicVolume => GameController.Instance.useProfile.OnMusic ? 1 : 0;

    public float SoundVolume => GameController.Instance.useProfile.OnSound ? 1 : 0;
