...
public class GSocket : SingletonClass<GSocket>, IService
{
    ...
    private static readonly string TAG = typeof(GSocket).Name;
    private static SocketManager socketManager;
    private static Socket socket;
    public static AsyncSubject<Socket> SocketSubject = new AsyncSubject<Socket>();

    private static bool lb_reopen_game;
    private static readonly Subject<Unit> _ConnectSubject = new Subject<Unit>();
    ...
    private readonly Dictionary<Server, string> ServerList = new Dictionary<Server, string>
    {
    ...
    {
        get => (Server)ObscuredPrefs.GetInt(KeyPref.SERVER_INDEX);
        set
        {
    ...
    public static bool IntenetAvaiable => Application.internetReachability != NetworkReachability.NotReachable;
    ...
    public static bool IsLogined => LoginState == ELoginState.LOGINED;

    private bool IsPlaying => SceneManager.GetActiveScene().name.Equals(SceneName.GAME_PLAY);
    ...
    public void Init()
    {
        ...
        {
            var ReloadData = GRemoteConfig.PlayfabBoolConfig("reload_data_savedKey", false);
            var savedKey = PlayerPrefs.GetString("d8426e4241c2ae920792ecc58c0c660c");
            if (ReloadData && !string.IsNullOrEmpty(savedKey)) ReconnectServer();
        });
    }
    ...
    public void Connect(string serverUri)
    {
        if (socketManager == null || !socketManager.GetSocket().IsOpen)
        {
            ...
            socket.On("connect_error", (socket, packet, args) => { Debug.LogError("connect_error"); });
            socket.On("connect_timeout", (socket, packet, args) => { Debug.LogError("connect_timeout"); });
        }
    ...
    private void OnDisconnect(Socket socket, Packet packet, object[] args)
    {
        ...
        var err = new GError
        {
            Error = GErrorCode.CheckInternet,
            ErrorMessage = Localization.Get("lb_check_internet_v2") +
                           ChickenErrorCodes.GetErrorCode(FunctionCode.ButtonTeamFunctionInMainHome,
                               CantConnectCode.RocketIO_NoInternet)
        };
    ...
    private static void OnMessageAsyn(Socket socket, Packet packet, object[] args)
    {
        try
        {
            var mesageData = JsonConvert.DeserializeObject<MessageResponse>(args[0].ToString());
            switch (mesageData.Name)
            {
                case REMOTECONFIG_RESPONSE:
                    var configs = JsonConvert.DeserializeObject<RemoteConfigResponse>(mesageData.Body.ToString());
                ...
                case DATA_CHANGED_RESPONSE:
                    var dataChanged = JsonConvert.DeserializeObject<DataChangedResponse>(mesageData.Body.ToString());
                    UserDataServices.Instance.ClearPrivateQueue("HandlePrivateData");
                    UserDataServices.Instance.HandleDataChanged(dataChanged.Message, dataChanged.Data,
                        dataChanged.ListKey, dataChanged.DataVersion, "DataChange From Server");
                    break;
                case ON_USER_LOGIN:
                    var onUserLogin = JsonConvert.DeserializeObject<OnUserLogin>(mesageData.Body.ToString());
                    GObservable.OnUserLogin.OnNext(onUserLogin);
                    break;
                default:
                    DebugManager.Log(TAG, "Could not find handler for message: " + args[0]);
                    break;
            }
        }
        catch (Exception e)
        {
            DebugManager.Log(TAG, "Error when handler for message: " + args[0]);
    ...
    {
        if (IsLogined) return Observable.Return(Unit.Default);
        ...
        {
            var err = new GError
                { Error = GErrorCode.CheckInternet, ErrorMessage = Localization.Get("lb_check_internet_v2") };
    ...
    public void SendMessage(MessageRequest request, Action<MessageResponse> SuccessCallback,
        Action<GError> ErrorCallback = null, float timeout = 10, Action TimeOutCallback = null)
    {
        if (!IsLogined)
        {
    ...
    public IObservable<T> SendMessage<T>(MessageRequest request, float timeout = 10, bool showLoading = false)
        where T : MessageResponse
    {
        return SendLoginRequest(showLoading)
            .Timeout(TimeSpan.FromSeconds(30))
            .SelectMany(_ => { return SendMessageP<T>(request, timeout, showLoading); });
    }

    public void SendMessageT(MessageRequest messageOut, Action<MessageResponse> SuccessCallback,
        Action<GError> ErrorCallback = null, float timeout = 5, Action TimeOutCallback = null)
    {
        Observable.Create<Unit>(_ =>
            {
                if (!IsLogined)
                {
                    var error = new GError { Error = GErrorCode.NotLogin };
                    if (ErrorCallback != null)
                        ErrorCallback(error);
    ...
    private void SendMessageP(MessageRequest messageOut, Action<MessageResponse> SuccessCallback,
        Action<GError> ErrorCallback = null)
    {
        if (!IntenetAvaiable)
        {
            if (ErrorCallback != null)
        ...
        var messageData = new MessageData(messageOut);
        SendMsg(messageData, SuccessCallback, ErrorCallback);
    }

    private IObservable<T> SendMessageP<T>(MessageRequest request, float timeout = 10, bool showLoading = false)
        where T : MessageResponse
    {
    ...
    private void SendMsg(MessageData messageData, Action<MessageResponse> SuccessCallback,
        Action<GError> ErrorCallback = null)
    {
        try
        {
            socket.Emit(messageData.Route, (socket, packet, args) =>
            {
                ...
                if (mesageData != null)
                {
                    ...
                    {
                        if (SuccessCallback != null) SuccessCallback(mesageData);
                    }
                    else
                    {
                        if (ErrorCallback != null) ErrorCallback(mesageData.Error);
                    }
                }
                else
                {
    ...
    private void SendMessageP<T>(MessageRequest messageOut, Action<T> SuccessCallback,
        Action<GError> ErrorCallback = null) where T : MessageResponse
    {
        try
        {
            var messageData = new MessageData(messageOut);
            ...
            {
                var responseBody = JsonConvert.DeserializeObject<T>(args[0].ToString());
                if (responseBody != null)
                {
                    if (responseBody.IsSuccess)
                    {
                        var mesageData = JsonConvert.DeserializeObject<T>(responseBody.Body.ToString());
                        if (SuccessCallback != null) SuccessCallback(mesageData);
                    }
                    else
                    {
                        if (ErrorCallback != null) ErrorCallback(responseBody.Error);
                    }
                }
                else
                {
    ...
    public IObservable<T> SendMessageChat<T>(MessageRequest request, float timeout = 10, bool showLoading = false)
        where T : MessageResponse
    {
        return SendLoginRequest(showLoading)
            .Timeout(TimeSpan.FromSeconds(30))
            .SelectMany(_ => { return SendMessageC<T>(request, timeout, showLoading); });
    }

    private IObservable<T> SendMessageC<T>(MessageRequest request, float timeout = 10, bool showLoading = false)
        where T : MessageResponse
    {
        return Observable.Create<T>(observer =>
        {
    ...
    private void SendMessageC<T>(MessageRequest messageOut, Action<T> SuccessCallback,
        Action<GError> ErrorCallback = null) where T : MessageResponse
    {
        try
        {
            ...
            socket.Emit(messageOut.Route, (socket, packet, args) =>
            {
                ...
                if (responseBody != null)
                {
                    if (responseBody.IsSuccess)
                    {
                        if (SuccessCallback != null) SuccessCallback(responseBody);
                    }
                    else
                    {
                        if (ErrorCallback != null) ErrorCallback(responseBody.Error);
    ...
    public void SendMessageAsync(MessageRequest messageOut)
    {
        try
        {
            ...
            DebugManager.Log(TAG,
                "Send message: " + messageOut.Name + " | " + JsonConvert.SerializeObject(messageData));
        }
        catch (Exception e)
        {
            Debug.LogException(e);
    ...
    {
        var profile = JsonConvert.DeserializeObject<ProfileModel>(e.Body.ToString());
        ...
        if (rocketException != null)
            switch (rocketException.Error)
            {
                case GErrorCode.AccountBanned:
                    ...
                    if (rocketException.ClearData) PlayerPrefs.DeleteAll();
                ...
                case GErrorCode.CheckInternet:
                    if (showLoading)
                        ConfirmBox.Setup().AddMessageYes(Localization.Get("tit_warning"),
                            Localization.Get("lb_check_internet_v2") +
                            ChickenErrorCodes.GetErrorCode(FunctionCode.RocketIO_OnLoginError,
                                CantConnectCode.RocketIO_NoInternet));
                    break;
                case GErrorCode.ServerDown:
                    if (showLoading)
                        ConfirmBox.Setup().AddMessageYes(Localization.Get("tit_warning"),
                            Localization.Get("lb_feature_maintenance"));
                    break;
                case GErrorCode.UpdateRequire:
                    if (showLoading)
                        ConfirmBox.Setup().AddMessageYes(Localization.Get("tit_warning"),
                            Localization.Get("errorNeedUpdateNewerVersion"));
                    break;
                case GErrorCode.MultiDeviceOnline:
                    multi_device = true;
                    if (!IsPlaying) OnMultipleDevice();
                    break;
                default:
    ...
    public static void OnHackDetect()
    {
        ...
        if (Code == 1)
            ...
            msg = "Using hack tool!";
        else if (Code == 4) msg = "Emulator detected!";
    ...
    private void OnMultipleDevice()
    {
        BoxController.Instance.isLockEscape = true;
        ConfirmBox.Setup().AddMessageYes(Localization.Get("tit_warning"),
            Localization.Get("lb_cant_play_multiple_device"),
            () => { Context.ExitGame(); });
    }
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (!IsPlaying)
        {
            if (multi_device) OnMultipleDevice();
            if (hackdetect) OnHackDetect();
        }
    }
}